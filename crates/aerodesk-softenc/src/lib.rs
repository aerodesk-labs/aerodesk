//! 共享软编：H.264 x264 编码器（AnnexB，4:2:0 WebRTC/VideoToolbox 兼容）。
//!
//! 供 macOS/Windows/Linux 适配器作为硬件编码的回退路径（#3/#4）。
...
mod tests {
    ...
    fn converts_bgra_rgba() {
        ...
        let rgb2 = rgba_to_rgb(&rgba);
        assert_eq!(rgb2, vec![10, 20, 30, 40, 50, 60]);
    }
}

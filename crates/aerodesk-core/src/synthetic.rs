...
impl crate::platform::MediaSource for SyntheticSource {
    ...
    fn next_frame(&mut self) -> Result<Option<crate::platform::VideoFrame>, Self::Error> {
        let w = self.width;
        let h = self.height;
        let raw = self.next_frame_bgra().to_vec();
        Ok(Some(crate::platform::VideoFrame {
            platform: None,
            handle: None,
            raw: Some(raw),
            width: w,

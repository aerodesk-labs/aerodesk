...
use crate::android::viewer::ViewerSession;
    ...
    match ViewerSession::connect(&server, &room, force_relay == JNI_TRUE) {
        ...
        Err(e) => {
            ...
            if let Ok(mut f) = std::fs::File::create("/data/data/io.aerodesk.viewer/err.txt") {
                use std::io::Write;
                let _ = f.write_all(format!("ad_viewer_create error: {e}").as_bytes());
            }
            0
        }
    }
...
pub extern "system" fn Java_io_aerodesk_viewer_NativeBridge_viewerSendInput<'local>(
    ...
    let viewer = ptr as *mut crate::android::viewer::ViewerSession;
    if viewer.is_null() {
        return JNI_FALSE;
    }
...
use crate::android::publisher::PublisherSession;

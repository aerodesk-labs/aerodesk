...
SIGNAL_PORT=14501 SIGNAL_PLAIN_PORT=14503 SIP_UDP_PORT=5060 SFU_URL=http://127.0.0.1:14502 \
...
for _ in $(seq 1 50); do
    if nc -z 127.0.0.1 14502 2>/dev/null && grep -q "SIP/UDP 监听已起" /tmp/turn-e2e-sig.log 2>/dev/null; then break; fi
    sleep 0.2
done
sleep 0.3

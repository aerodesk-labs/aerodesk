...
SFU_PID=$!
SIP_UDP_PORT=5060 ./target/debug/aerodesk-signal >/tmp/disp-sig.log 2>&1 &
SIG_PID=$!
for _ in $(seq 1 50); do
    if grep -q "SIP/UDP 监听已起" /tmp/disp-sig.log 2>/dev/null; then break; fi
...
if grep -q "control: display switch request -> display 1" /tmp/disp-pub.log; then
    ...
    # #584 恢复 FAIL（80936e8 曾降级 WARN）：control 通道回归保护。

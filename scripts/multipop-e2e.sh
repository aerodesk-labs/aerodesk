...
SFU_B=$!
SIP_UDP_PORT=5060 POP_ID=pop-a ROOM_POP_MAP="eu-=pop-b" POP_URLS="pop-b=ws://127.0.0.1:3006/ws" ./target/debug/aerodesk-signal >/tmp/mpop-sig-a.log 2>&1 &
SIG_A=$!
SIP_UDP_PORT=5060 POP_ID=pop-b ROOM_POP_MAP="eu-=pop-b" POP_URLS="pop-a=ws://127.0.0.1:3001/ws" SIGNAL_PORT=3004 SIGNAL_PLAIN_PORT=3006 SFU_URL=http://127.0.0.1:3007 ./target/debug/aerodesk-signal >/tmp/mpop-sig-b.log 2>&1 &
SIG_B=$!

for _ in $(seq 1 50); do
    if nc -z 127.0.0.1 3002 2>/dev/null && nc -z 127.0.0.1 3007 2>/dev/null \
       && grep -q "SIP/UDP 监听已起" /tmp/mpop-sig-a.log 2>/dev/null && grep -q "SIP/UDP 监听已起" /tmp/mpop-sig-b.log 2>/dev/null; then
        break
    fi
    sleep 0.2
done

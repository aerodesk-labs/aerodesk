...
[ "$ok" = "1" ] || fail "PoP-A publisher 未注册"; echo "  publisher registered"
...
[ "$ok" = "1" ] || fail "bridge 未就绪（双腿未连）"; echo "  bridge 双腿已连"
ok=0
for _ in $(seq 1 60); do
  grep -q "ICE connected" /tmp/bridge-pub-a.log 2>/dev/null && ok=1 && break
  sleep 0.5
done
[ "$ok" = "1" ] || fail "PoP-A publisher 未与 bridge view 腿接通"; echo "  A-publisher 已接通 bridge"

echo "== PoP-B：启动 viewer（呼入同房间，期望不经 Redirect 看到 PoP-A 媒体）"
AERO_SIP_PORT="$SIP_B" "$TARGET_DIR/aerodesk-agent" --role viewer --signal "ws://127.0.0.1:${PLAIN_B}" --room "$ROOM" \
...
AERO_SIP_PORT="$SIP_A" "$TARGET_DIR/aerodesk-agent" --role publisher --recv-dir "$OUT_DIR" \

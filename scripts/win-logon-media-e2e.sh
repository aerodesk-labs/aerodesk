...
# 前置:cargo build -p aerodesk-host -p aerodesk-signal -p aerodesk-sfu
...
echo "== viewer 收流断言"
OUT=$(timeout 12 "$BIN" --role viewer --signal ws://127.0.0.1:3003 --room "$ROOM" 2>&1 | grep "RECEIVED" | tail -1)
echo "$OUT"

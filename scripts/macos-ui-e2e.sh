...
SFU=$!
"$ROOT/target/debug/aerodesk-signal" >/tmp/macosui-sig.log 2>&1 &
SIG=$!
OK=0
...
if [ "$OK" != "1" ]; then echo "FAIL: SFU/signal 未就绪"; tail -10 /tmp/macosui-sfu.log; exit 1; fi
...
RUST_LOG=debug "$ROOT/target/debug/aerodesk-desktop" \
...
for i in range(60):  # 最多 60s
    try:
        txt = open('/tmp/macosui-ui.log', errors='replace').read()
...
if not ok:
    ...
    print(open('/tmp/macosui-ui.log', errors='replace').read()[-1500:])
    sys.exit(1)
PY

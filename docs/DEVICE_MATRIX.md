...
| #271 剪贴板 | **macOS/Windows/Linux 文本双向同步**（#281/#283 + 既有 macOS）；**图片双向同步**（#503-2：协议 FileKind::ClipboardImage + core 分片内存接收，Windows 原生 DIB / macOS NSPasteboard PNGf / Linux xclip/wl-copy；CLI 与 desktop 被控端（generic/macos publisher）均已接线——file 通道接收落地 + 1s 轮询回传，图片优先、防回声；`scripts/clipboard-e2e.sh` 文本+图片双向断言）；富文本待做 | — |
